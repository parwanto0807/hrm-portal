...
export const AttendanceTable = ({
    ...
    const columns = getColumns(onEdit, onView, holidays, isEmployee);
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-slate-800 mt-4 px-2">
                ...
                <div className="hidden sm:flex items-center justify-between gap-4">
                    <div className="flex items-center gap-2">
                        <span className="text-sm text-slate-500 dark:text-slate-400 font-medium">Rows per page</span>
                        <Select
                            value={pagination.limit.toString()}
                            onValueChange={(value) => onLimitChange(parseInt(value))}
                        >
                            <SelectTrigger className="h-9 w-[70px] bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
                        ...
                        <div className="text-sm text-slate-500 dark:text-slate-400 ml-4 font-medium">
                            Showing <span className="text-slate-900 dark:text-white font-bold">{(pagination.page - 1) * pagination.limit + 1}</span> to <span className="text-slate-900 dark:text-white font-bold">{Math.min(pagination.page * pagination.limit, pagination.total)}</span> of <span className="text-slate-900 dark:text-white font-bold">{pagination.total}</span>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center justify-center text-[11px] font-black uppercase tracking-widest min-w-[100px] bg-slate-100 dark:bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-200/50 dark:border-slate-800/50 text-slate-600 dark:text-slate-400">
                ...
                <div className="flex sm:hidden flex-col gap-4 py-2">
                    <div className="flex items-center justify-between">
                        <div className="bg-slate-100 dark:bg-slate-900 px-4 py-2 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                            <p className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-500 dark:text-slate-400">
                                {(pagination.page - 1) * pagination.limit + 1}-{Math.min(pagination.page * pagination.limit, pagination.total)} <span className="mx-1.5 opacity-30">/</span> {pagination.total}
                            </p>
                        </div>
                        <div className="bg-sky-50 dark:bg-sky-950/20 px-4 py-2 rounded-2xl border border-sky-100 dark:border-sky-900/40 shadow-sm">
                            <p className="text-[10px] font-black uppercase tracking-[0.1em] text-sky-600 dark:text-sky-400">
                                Page {pagination.page} <span className="mx-1.5 opacity-30">of</span> {pagination.totalPages}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <Button
                            variant="outline"
                            className="h-14 rounded-2xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 font-black text-xs uppercase tracking-widest shadow-sm active:scale-[0.97] transition-all flex items-center justify-center gap-2 hover:bg-slate-50 dark:hover:bg-slate-900"
                            onClick={() => onPageChange(pagination.page - 1)}
                            disabled={pagination.page === 1 || isLoading}
                        >
